"""
create_vitis.py - Crea workspace Vitis para P_102 bram_ctrl_v2
...
if os.path.exists(os.path.join(ws_dir, "bram_ctrl_v2_platform")):
    print("Platform already exists, rebuilding app...")
    client = vitis.create_client(workspace=ws_dir)
    app_src_dir = os.path.join(ws_dir, "bram_ctrl_v2_test", "src")
    os.makedirs(app_src_dir, exist_ok=True)
    shutil.copy2(app_src, app_src_dir)
    comp = client.get_component("bram_ctrl_v2_test")
    comp.build()
    elf_path = os.path.join(ws_dir, "bram_ctrl_v2_test", "build", "bram_ctrl_v2_test.elf")
    print(f"\n  ELF: {elf_path}")
    sys.exit(0)
...
# Create app
print("[3/4] Creando app bram_ctrl_v2_test ...")
xpfm_path = os.path.join(ws_dir, "bram_ctrl_v2_platform", "export",
                          "bram_ctrl_v2_platform", "bram_ctrl_v2_platform.xpfm")
app = client.create_app_component(
    name="bram_ctrl_v2_test",
    platform=xpfm_path,
