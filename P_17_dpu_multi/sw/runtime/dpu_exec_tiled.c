                        ...
                        dpu_prof_t    *prof)
{
    ...
    for (int oh0 = 0; oh0 < L->h_out; oh0 += H_TILE) {
        ...
        for (int ow0 = 0; ow0 < L->w_out; ow0 += W_TILE) {
            ...
            /* Copiar sub-input tensor NHWC contiguo */
            for (int rr = 0; rr < in_h_real; rr++) {
                const uint8_t *src = in_ddr +
                                     ((ih_lo + rr) * L->w_in + iw_lo) * L->c_in;
                uint8_t *dst = tile_in_buf + rr * in_w_real * L->c_in;
            ...
            /* Copiar tile_out al output tensor grande */
            for (int rr = 0; rr < h_tile; rr++) {
                const uint8_t *src = tile_out_buf + rr * w_tile * L->c_out;
                uint8_t *dst = out_ddr +
                               ((oh0 + rr) * L->w_out + ow0) * L->c_out;
