# ==============================================================
# create_bd.tcl - Block Design: Zynq PS + AXI DMA + stream_adder
# ZedBoard (xc7z020clg484-1)
#
# Arquitectura:
#   Zynq PS --M_AXI_GP0--> AXI IC --> AXI DMA (S_AXI_LITE)
#                                  --> stream_adder (S_AXI_LITE)
#   AXI DMA M_AXIS_MM2S --> stream_adder s_axis
#   stream_adder m_axis  --> AXI DMA S_AXIS_S2MM
#   AXI DMA --M_AXI_MM2S/S2MM--> AXI IC --> Zynq PS (S_AXI_HP0) --> DDR
#
# Flujo: ARM escribe add_value via AXI-Lite en stream_adder,
#        DMA lee datos de DDR (MM2S) -> stream_adder suma -> DMA escribe (S2MM)
# ==============================================================

# --- Create Block Design ---
create_bd_design "zynq_adder_bd"

# ==============================================================
# 1. Add RTL sources (stream_adder + dependencies)
# ==============================================================
...
read_vhdl [file join $src_dir stream_adder.vhd]
...
# ==============================================================
# 4. stream_adder (our RTL module)
# ==============================================================
create_bd_cell -type module -reference stream_adder stream_adder_0

# ==============================================================
# 5. AXI Interconnects
# ==============================================================

# GP0 interconnect: Zynq GP0 -> DMA S_AXI_LITE + stream_adder S_AXI
...
connect_bd_net [get_bd_pins ps7/FCLK_CLK0] \
    ...
    [get_bd_pins stream_adder_0/clk] \
...
connect_bd_net [get_bd_pins proc_sys_reset_0/peripheral_aresetn] \
    ...
    [get_bd_pins stream_adder_0/resetn]

# --- AXI GP0: Zynq -> IC -> DMA control (M00) + stream_adder config (M01) ---
...
connect_bd_intf_net [get_bd_intf_pins axi_ic_gp0/M01_AXI] \
    [get_bd_intf_pins stream_adder_0/S_AXI]
...
# --- DMA Stream -> stream_adder -> DMA (en vez de loopback) ---
connect_bd_intf_net [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S] \
    [get_bd_intf_pins stream_adder_0/s_axis]
connect_bd_intf_net [get_bd_intf_pins stream_adder_0/m_axis] \
...
# Generate wrapper
make_wrapper -files [get_files zynq_adder_bd.bd] -top
set bd_dir [file dirname [get_files zynq_adder_bd.bd]]
set wrapper_file [file normalize "$bd_dir/hdl/zynq_adder_bd_wrapper.v"]
add_files -norecurse $wrapper_file
set_property top zynq_adder_bd_wrapper [current_fileset]
update_compile_order -fileset sources_1

puts "OK: Block Design 'zynq_adder_bd' creado"
puts "  - Zynq PS con DDR (512MB)"
puts "  - AXI DMA (MM2S + S2MM)"
puts "  - stream_adder entre MM2S y S2MM"
puts "  - AXI-Lite: DMA en M00, stream_adder en M01"
