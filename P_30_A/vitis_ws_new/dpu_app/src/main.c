/*
 * main.c -- P_30_A DPU + Ethernet TCP bare-metal entry point.
 *
 * Same structure as P_18 but calls dpu_v4_init() for the dual-DMA
 * architecture (DMA_IN for input+bias, DMA_W for weights via FIFO).
 */
...
int main(void)
{
    ...
    xil_printf("\r\n\r\n##############################################\r\n");
    xil_printf("  P_30_A DPU+Ethernet server (conv_v4)\r\n");
    xil_printf("##############################################\r\n");
    ...
    /* Enable IRQ mask AFTER netif is up */
    ...
    /* Init DPU v4 (dual DMA: DMA_IN + DMA_W) */
    int dpu_r = dpu_v4_init();
    if (dpu_r != DPU_OK) {
        xil_printf("ERROR: dpu_v4_init=%d\r\n", dpu_r);
    } else {
        xil_printf("DPU v4 init OK (dual DMA)\r\n");
    }

    /* Also init the legacy DMA path for leaky/pool/add */
    int legacy_r = dpu_init();
    if (legacy_r != DPU_OK) {
        xil_printf("WARN: dpu_init (legacy)=%d\r\n", legacy_r);
    }
